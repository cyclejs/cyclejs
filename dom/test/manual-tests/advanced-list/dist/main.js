...
},{"./ScopeChecker":5,"./utils":18,"matches-selector":40}],2:[function(require,module,exports){
"use strict";
...
},{"@cycle/xstream-adapter":19,"xstream":73}],4:[function(require,module,exports){
"use strict";
...
},{"./ElementFinder":1,"./EventDelegator":2,"./fromEvent":7,"./isolate":11,"./utils":18,"@cycle/xstream-adapter":19,"matches-selector":40,"xstream":73}],5:[function(require,module,exports){
"use strict";
 ...
 * output of this driver is a "DOMSource": a collection of Observables queried
 * the methods `select()` and `events()`.
 *
 ...
 * the app on the DOM. The `events()` function also allows you to specify the
 * an options argument with `useCapture` parameter of the event listener. That
 * is, the full function signature is `events(eventType, options)` where
 * `options.useCapture` is by default `false`.
 *
...
},{"./hyperscript":9,"./hyperscript-helpers":8,"./makeDOMDriver":13,"./makeHTMLDriver":14,"./mockDOMSource":15,"snabbdom/thunk":60}],11:[function(require,module,exports){
"use strict";
...
},{"./MainDOMSource":4,"./VNodeWrapper":6,"./isolateModule":12,"./modules":16,"./transposition":17,"./utils":18,"@cycle/xstream-adapter":19,"snabbdom":59,"xstream":73}],14:[function(require,module,exports){
"use strict";
...
},{"./HTMLSource":3,"./transposition":17,"@cycle/xstream-adapter":19,"snabbdom-to-html":44}],15:[function(require,module,exports){
"use strict";
...
},{"@cycle/xstream-adapter":19,"xstream":73}],16:[function(require,module,exports){
"use strict";
...
},{"dup":19,"xstream":68}],65:[function(require,module,exports){
"use strict";
...
},{"@cycle/base":62,"@cycle/xstream-adapter":64}],66:[function(require,module,exports){
"use strict";
...
function intent(DOM, tickerAction$) {
  var tickerRemove$ = tickerAction$.filter(function (a) {
    return a.type === 'remove';
  });
  var action$ = _xstream2.default.merge(tickerRemove$);
  return action$;
}

function model(action$, TickerComponent) {
  var insertReducer$ = _xstream2.default.periodic(5000).take(10).map(function (id) {
    return function insertReducer(oldList) {
      var color$ = _xstream2.default.periodic(1000).map(makeRandomColor).startWith('#000000');
      var out = TickerComponent(color$, id);
      return oldList.concat([{ id: id, DOM: out.DOM, action$: out.action$ }]);
    };
  });

  var removeReducer$ = action$.filter(function (a) {
    return a.type === 'remove';
  }).map(function (action) {
    return function removeReducer(oldList) {
  ...
  var list$ = _xstream2.default.merge(insertReducer$, removeReducer$).fold(function (oldList, reducer) {
    return reducer(oldList);
  }, []);
...
function view(children$) {
  ...
  var loading = (0, _index.h3)('Loading...');
  return children$.map(function (children) {
    return(
      // childrenStreams.length > 0 ?
      //   xs.combine(...childrenStreams).map(children => div('#the-view', children)) :
      //   xs.of(div('#the-view', [loading]))
      (0, _index.div)('#the-view', children.length > 0 ? children : [loading])
    );
  });
}

var TickerWrapper = function TickerWrapper(sources) {
  return function TickerComponent(color$, id) {
    var ticker = (0, _isolate2.default)(_Ticker2.default)({ DOM: sources.DOM, color: color$ });
    return {
      DOM: ticker.DOM,
      action$: ticker.action$.map(function (a) {
        a.id = id;
        return a;
      })
    };
...
function App(sources) {
  var tickerProxyAction$ = _xstream2.default.create();
  var action$ = intent(sources.DOM, tickerProxyAction$);
  var tickers$ = model(action$, TickerWrapper(sources));
  var tickerViews$ = tickers$.map(function (list) {
    return list.map(function (t) {
      return t.DOM;
    });
  });
  var tickerAction$ = tickers$.map(function (list) {
    return _xstream2.default.merge.apply(_xstream2.default, _toConsumableArray(list.map(function (t) {
      return t.action$;
    })));
  }).flatten();
  tickerProxyAction$.imitate(tickerAction$);
  var vtree$ = view(tickerViews$);
  var sinks = {
    DOM: vtree$
  };
  return sinks;
...
},{"../../../../lib/index":10,"./Ticker.js":71,"@cycle/isolate":63,"xstream":68}],70:[function(require,module,exports){
'use strict';
...
var main = _App2.default;

(0, _xstreamRun.run)(main, {
  DOM: (0, _index.makeDOMDriver)('#main-container', { transposition: true })
});

},{"../../../../lib/index":10,"./App.js":69,"@cycle/xstream-run":65}],71:[function(require,module,exports){
'use strict';
...
function intent(DOMSource) {
  var removeClicks$ = DOMSource.select('.remove-btn').events('click');
  var stop$ = removeClicks$;
  var remove$ = removeClicks$.compose((0, _delay2.default)(500)).take(1);
  return _xstream2.default.merge(stop$.mapTo({ type: 'stop' }), remove$.mapTo({ type: 'remove' }));
}

function model(action$, givenColor$) {
  var stop$ = action$.filter(function (a) {
    return a.type === 'stop';
  });
  var x$ = _xstream2.default.periodic(50).startWith(0).endWhen(stop$);
  var y$ = _xstream2.default.periodic(100).startWith(0).endWhen(stop$);
  var color$ = _xstream2.default.merge(givenColor$.endWhen(stop$), stop$.mapTo('#FF0000'));
  return _xstream2.default.combine(x$, y$, color$).map(function (_ref) {
...
function view(state$) {
  return state$.map(function (_ref3) {
    ...
    var style = { color: color, backgroundColor: '#ECECEC' };
    return (0, _index.div)('.ticker', { style: style }, [(0, _index.h4)('x' + x + ' ' + color), (0, _index.h1)('Y' + y + ' ' + color), (0, _index.button)('.remove-btn', 'Remove')]);
  });
}

function Ticker(sources) {
  var action$ = intent(sources.DOM);
  var state$ = model(action$, sources.color);
...
},{"../../../../lib/index":10,"xstream":68,"xstream/extra/delay":67}],72:[function(require,module,exports){
"use strict";
...
var Stream = function () {
    ...
    Stream.prototype._onlyReachesThis = function (x) {
        if (x.out === this) {
            ...
            for (var i = 0, N = x._ils.length; i < N; i++) {
                if (!this._onlyReachesThis(x._ils[i])) return false;
            }
            return true;
        } else {
            return false;
