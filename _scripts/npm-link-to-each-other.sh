...
for d in "${packages[@]}"; do
  ...
  for d2 in "${packages[@]}"; do
    if `echo ${deps} | grep "cycle-monorepo-sandbox-${d2}" 1>/dev/null 2>&1`; then
      echo "> npm link $d2";
      npm link cycle-monorepo-sandbox-$d2;
    fi
    if `echo ${devdeps} | grep "cycle-monorepo-sandbox-${d2}" 1>/dev/null 2>&1`; then
      echo "> npm link $d2";
      npm link cycle-monorepo-sandbox-$d2;
    fi
  done
